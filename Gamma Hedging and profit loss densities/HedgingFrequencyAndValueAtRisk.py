...
def NTrading(tr):
    ...
    for i in range(Nmc):
        S[0][i]=1
        B[0][i]=1
        A[0][i]=DeltaFunc(0,1,K,T,r,Sigma2[0])
        P[0][i]=A[0][i]*S[0][i]+B[0][i]
        V[0][i]=Call_BS(0,1,K,T,r,Sigma2[0])
    ...
    for j in range(Nmc):
        for i in range(0,N-1):
            S[i+1][j]=S[i][j]*np.exp((r-0.5*Sigma2[i]**2)*dt+Sigma2[i]*np.sqrt(dt)*np.random.normal())
        
        
        
        for i in range (0,N-1):
            if  (i+1)%tr==0:
              A[i+1][j]=DeltaFunc(t[i+1],S[i+1][j],K,T,r,Sigma2[i])
              B[i+1][j]=(A[i][j]-A[i+1][j])*S[i+1][j]+B[i][j]*(1+r*dt)
              P[i+1][j]=A[i+1][j]*S[i+1][j]+B[i+1][j]
              V[i+1][j] =Call_BS(t[i+1],S[i+1][j],K,T,r,Sigma2[i])   
    ...
    plt.show()
    sns.distplot(data)
    plt.title('Probability density function for modulo '+str(tr))
    plt.show()
    plt.plot(t,V,t,Pact)
    plt.xlabel("Time")
    plt.ylabel("Value")
