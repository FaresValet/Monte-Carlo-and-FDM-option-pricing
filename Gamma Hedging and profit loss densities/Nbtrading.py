...
def NTrading(tr):
    ...
    for i in range(Nmc):
        ...
        S[0][i]=1
        B[0][i]=1
        A[0][i]=DeltaFunc(0,1,K,T,r,p)
        P[0][i]=A[0][i]*S[0][i]+B[0][i]
        V[0][i]=Call_BS(0,1,K,T,r,p)
    ...
    for j in range(Nmc):
        for i in range(0,N-1):
            p=StochEin()
            S[i+1][j]=S[i][j]*np.exp((r-0.5*p**2)*dt+p*np.sqrt(dt)*np.random.normal())
        ...
        for i in range (0,N-1):
            if  (i+1)%tr==0:
              p=StochEin()
              A[i+1][j]=DeltaFunc(t[i+1],S[i+1][j],K,T,r,p)
              B[i+1][j]=(A[i][j]-A[i+1][j])*S[i+1][j]+B[i][j]*(1+r*dt)
              P[i+1][j]=A[i+1][j]*S[i+1][j]+B[i+1][j]
              V[i+1][j] =Call_BS(t[i+1],S[i+1][j],K,T,r,p)   
